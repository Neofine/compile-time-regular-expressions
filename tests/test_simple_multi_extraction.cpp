...
int main() {
    std::cout << "=== Simple Multi-Path Extraction Tests ===\n\n";
    ...
    // Sequence with char class: [0-2]test
    {
    ...
    // Prefix + char class + suffix: x[0-1]y
    {
        using AST = ctre::sequence<
            ctre::character<'x'>,
            ctre::char_range<'0', '1'>,
        ...
        if (result.count == 2) {
            ...
            for (size_t i = 0; i < 2; ++i) {
                if (result.literals[i].length == 3 && 
                    result.literals[i].chars[0] == 'x' && 
                    result.literals[i].chars[2] == 'y') {
