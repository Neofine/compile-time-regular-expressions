...
struct range_info<char_range<A, B>> {
    static constexpr char min = A, max = B;
};
template <auto C>
struct range_info<character<C>> {
    static constexpr char min = C, max = C;
};
...
template <typename T>
inline constexpr bool is_char_range_type_v = is_char_range_type<T>::value;
...
struct is_multi_range<set<Ranges...>> : std::true_type {
    ...
    template <size_t N>
    [[nodiscard]] static consteval char get_min() noexcept {
        return range_info<std::tuple_element_t<N, std::tuple<Ranges...>>>::min;
    }
    template <size_t N>
    [[nodiscard]] static consteval char get_max() noexcept {
        return range_info<std::tuple_element_t<N, std::tuple<Ranges...>>>::max;
    }
};

template <typename T>
inline constexpr bool is_multi_range_v = is_multi_range<T>::value;
template <typename T>
inline constexpr bool is_valid_multi_range_v = is_multi_range<T>::is_valid;

template <typename T>
struct is_two_range : std::false_type {};
template <typename... R>
struct is_two_range<set<R...>>
...
[[nodiscard]] inline Iterator match_n_range_sse_impl(Iterator current, EndIterator last, size_t& count,
    ...
    while (last - current >= 16) {
        ...
        ((result = _mm_or_si128(
              result,
              check_range_sse(data, static_cast<unsigned char>(is_multi_range<PatternType>::template get_min<Is>()),
...
[[nodiscard]] inline Iterator match_n_range_avx2_impl(Iterator current, EndIterator last, size_t& count,
    ...
    while (last - current >= 32) {
        ...
        ((result = _mm256_or_si256(
              result,
              check_range_avx2(data, static_cast<unsigned char>(is_multi_range<PatternType>::template get_min<Is>()),
                               static_cast<unsigned char>(is_multi_range<PatternType>::template get_max<Is>())))),
         ...);
